...
3. `sample_rows(resource_id, n=5)` — **discovery only.** Use this to see
   actual cell shapes, code values, and units. Rows come back in
   `record_number` ascending order — that's the row's insertion sequence,
   not a meaningful sort. **Never use `sample_rows` to answer a "first /
   top / earliest N rows" question** — that's `query_datastore` with a
   `sorts` clause (see "First N rows" below).
...
- **Three identifier formats — use the right one for each tool.**
  Dataset UUIDs (`d460252e-...`, from `list_datasets` /
  `search_datasets`) are **not** resource_ids. Passing one to any
  datastore tool (`query_datastore`, `query_datastore_join`,
  `get_datastore_schema`, `sample_rows`, `distinct_values`,
  `get_datastore_stats`) fails with "Could not resolve resource".
  Datastore tools need a resource_id (`{hash}__{version}`, e.g.
  `046ab6a6…__1773329007`) — get it from `find_dataset_resources(title)`
  or `list_distributions(dataset_ref)`. All ID-taking tools also accept
  a dataset title as a fuzzy fallback, and `list_distributions` accepts
  any of the three (UUID, resource_id, title).
- **Use only known columns.** Every column name in a `query_datastore`
  ...
  that resource. **Column names are case-sensitive — copy them exactly,
  don't retype from memory** (`Violent_Crimes` ≠ `violent_crimes`). If
  the user's wording maps to a column synonym (e.g. "homicides" →
  `murder_and_nonnegligent_manslaughter`), confirm via the schema or a
  column caveat — don't guess.
- **Disambiguate when `find_dataset_resources` returns `multiple_matches`.**
  The response then has no `dataset_id` — instead, a `multiple_matches`
  array of `{dataset_id, title}` candidates plus a `refine_hint`. Don't
  pick blindly. If exactly one candidate plausibly matches the user's
  intent, name it back ("I found two datasets matching X — did you mean
  Y or Z?") and let them pick. If you can refine the search term
  yourself (e.g. they said "crime" and the candidates are obviously
  about specific cities), call `find_dataset_resources` again with the
  narrower term. Never pass any candidate's UUID to a downstream tool
  until the user has confirmed — silently picking the first one is the
...
- **Use dictionary metadata when present.** `get_datastore_schema`
  ...
  both exist, both apply. **Don't call `get_data_dictionary` after
  `get_datastore_schema`** — the schema response already includes the
  dictionary's per-column titles, descriptions, and types, plus the
  `dictionary_url`. Only reach for `get_data_dictionary` when you
  specifically need the standalone dictionary fields list independent
  of any one resource.
...
substitute `sample_rows`** — sample_rows orders by `record_number` (the
row's insertion sequence, not a meaningful sort) and is reserved for
discovering cell shapes.
...
## Raw query (advanced — escape hatch only)

`query_datastore_raw(payload)` accepts a verbatim DKAN DatastoreQuery JSON
payload and returns the raw REST response. **Do not use it for queries the
flat tools can express — the vast majority of work belongs in
`query_datastore` / `query_datastore_join`.** Those tools provide
case-insensitive column matching, `sanity_flags`, and dictionary
enrichment; the raw tool returns none of that.

When raw is the right tool:

- **OR-grouped conditions / nested logic** — the flat `conditions` array
  is implicitly AND-joined. Use raw with a `groupOperator` group when the
  user asks for "X OR (Y AND Z)".
  `{"resources":[{"id":"<rid>","alias":"t"}],"conditions":[{"groupOperator":"or","conditions":[{"property":"state","value":"CA","operator":"="},{"groupOperator":"and","conditions":[{"property":"region","value":"West","operator":"="},{"property":"population","value":1000000,"operator":">"}]}]}]}`
- **Three-way joins** — `query_datastore_join` only supports two
  resources. For three, list them all in `resources[]` and add multiple
  `joins[]` entries.
- **Compound aggregate expressions** — sums of differences,
  conditional counts, anything that nests an expression inside another
  expression operand.
- **Selective payload control** — when you specifically need
  `{schema: true, results: false, count: true}` together, or any
  combination the flat tools' booleans don't expose.

When raw is **not** the right tool — use the flat tools instead:

- Plain filtered query, sort, limit, offset, group-by, single-aggregate.
- Single-resource queries on a known column.
- Anything where you'd otherwise call `query_datastore` and add one extra
  condition.

Raw responses do not include `sanity_flags`. To know whether a result
was truncated, inspect `count` vs `results.length` directly. There is no
`row_cap_hit`, `zero_rows`, or `coverage_warning` to lean on — read the
shape and report what you see.

`resources[].id` accepts a `{identifier}__{version}` resource id or a
fuzzy dataset title (resolved server-side). Validation errors come back
as `{error, validation_errors:[{pointer, keyword, args}]}` — fix the
payload against those pointers rather than guessing.
...
For charts spanning a long time series or many categories, you may
raise `query_datastore`'s `limit` above the default 100, up to 5000.
Keep `limit ≤ 500` for browsing and ad-hoc questions. The higher cap
exists so charts don't silently get truncated to the first 500 rows
and mislead the user about the actual data range. Always sort the
underlying query (e.g. by year ascending) before charting, and prefer
explicit `columns` so the chart spec only carries what it plots.
