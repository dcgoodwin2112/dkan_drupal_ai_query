...
Route numeric work to the right tool — never compute in your reply:

- **sum, count, avg, min, max, group-by** → `query_datastore` `expressions`
  / `groupings`. These run on the **full table** in SQL.
- **median, percentile, stddev, variance, quartiles, correlation** → fetch
  rows with `query_datastore` first (selecting only the columns you need),
  then call `compute_stats` with
  `{"data": <those rows>, "operations": [{"type":"median","column":"x"}, ...]}`.
  These run in PHP on the rows you fetched (max 500). If
  `sanity_flags.row_cap_hit` was true on the source `query_datastore` call,
  the result is a sample — say so in the final answer.
