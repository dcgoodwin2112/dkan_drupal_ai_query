# DKAN Drupal AI Query Mock

Scenario-driven mock AI provider for browser-driven testing of
`dkan_drupal_ai_query` without hitting a live LLM.
...
```bash
ddev drush en dkan_drupal_ai_query_mock
...
3. State key `dkan_drupal_ai_query_mock.active_scenario` (set via the picker
...
`dkan_drupal_ai_query_mock.fixture_resource_id`, written by
...
|---|---|---|
| `dkan_drupal_ai_query_mock.active_scenario` | unset | Force a specific scenario site-wide. |
| `dkan_drupal_ai_query_mock.force_active` | unset | Engage the emergency `PreGenerateResponseEvent` kill-switch (intercepts every chat call regardless of provider). |
| `dkan_drupal_ai_query_mock.turn_delay_ms` | 600 | Per-turn `usleep` to simulate LLM latency. The widget polls every 500 ms; without a deliberate stall, sub-second mock runs complete before any poll fires and the live UI doesn't show artifacts in real time. Set to 0 to disable. |
...
id from state key `dkan_drupal_ai_query_mock.fixture_resource_id` at chat-call
