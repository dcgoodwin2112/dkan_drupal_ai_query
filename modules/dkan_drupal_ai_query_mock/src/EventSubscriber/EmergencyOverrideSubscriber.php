...
namespace Drupal\dkan_drupal_ai_query_mock\EventSubscriber;
 ...
 * key `dkan_drupal_ai_query_mock.force_active` is set. Independent of which
...
class EmergencyOverrideSubscriber implements EventSubscriberInterface {
  ...
  public function onPreGenerate(PreGenerateResponseEvent $event): void {
    ...
    $message = new ChatMessage(
      'assistant',
      "_(dkan_drupal_ai_query_mock emergency override active.)_\n\nNo LLM call was made. Selected provider: `" . $event->getProviderId() . '`. ' .
      'Disable by unsetting the DKAN_AIQ_FORCE_MOCK env var or clearing state key `dkan_drupal_ai_query_mock.force_active`.',
    );
  ...
  private function isActive(): bool {
    ...
    return (bool) $this->state->get('dkan_drupal_ai_query_mock.force_active', FALSE);
  }
