...
namespace Drupal\dkan_ai_query_mock\Plugin\AiProvider;
...
/**
 * Scenario-driven mock provider for the dkan_ai_query widget.
 *
...
class DkanAiqMockProvider extends AiProviderClientBase implements ChatInterface {
  ...
  public function getConfig(): ImmutableConfig {
    return $this->configFactory->get('dkan_ai_query_mock.settings');
  }
  ...
  public function chat(array|string|ChatInput $input, string $model_id, array $tags = []): ChatOutput {
    ...
    $delay_ms = (int) (\Drupal::state()->get('dkan_ai_query_mock.turn_delay_ms', 600));
    if ($delay_ms > 0) {
  ...
  protected function scriptedTurn(ChatInput $input): array {
    $matcher = \Drupal::service('dkan_ai_query_mock.scenario_matcher');
    $scenario = $matcher->resolve($input);
    if ($scenario === NULL) {
      $question = $matcher->latestUserQuestion($input);
      $loader = \Drupal::service('dkan_ai_query_mock.scenario_loader');
      $available = array_map(
  ...
  protected function substitutePlaceholders(mixed $value): mixed {
    ...
    return preg_replace_callback('/\$\{([A-Z_][A-Z0-9_]*)\}/', function (array $m) {
      return match ($m[1]) {
        'FIXTURE_RESOURCE_ID' => (string) (\Drupal::state()->get('dkan_ai_query_mock.fixture_resource_id') ?? $m[0]),
        default => $m[0],
      };
    }, $value);
  }
